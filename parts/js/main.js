...
(function() {
  var assert, backlog, draw, draw_data, draw_hl_fade, draw_status, draw_status_counter, fill, focus, font_scale, h, layout, svg, tag_highlight, tag_links, tag_links_box, tags, vis, vis_bg, vis_box, w, _ref;
  ...
  draw_hl_fade = function(selection) {
    var hl_check;
    assert((selection != null) || draw_data);
    hl_check = function(d) {
  ...
  draw_status = function() {
    ...
    return d3.select('#vis-status div').style('width', ((draw_status_counter / tags.length) * 100) + '%');
  };
